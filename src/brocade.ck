...
public class Brocade extends GGen {
    150 => int NUM_STARS;
    10 => int TRAIL_LENGTH;
    Particle shell --> this;
    vec3 initial_pos;
    ...
    fun void init() {
        ...
        for (0 => int i; i < NUM_STARS; 1 +=> i) {
            stars[i] --> this;
            rand_unit_vec() * Math.random2f(0.0, 0.4) => new_velocities[i];
            stars[i].init(0.01 * shell.gravity, shell_mat, trail_pool, 300::ms, 3, 1);
        }
    }
