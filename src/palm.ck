...
public class Palm extends GGen {
    15 => int NUM_STARS;
    100 => int TRAIL_LENGTH;
    Particle shell --> this;
    vec3 initial_pos;
    ...
    fun void init() {
        ...
        for (0 => int i; i < NUM_STARS; 1 +=> i) {
            stars[i] --> this;
            rand_unit_vec() * Math.random2f(0.0, 0.75) => new_velocities[i];
            stars[i].init(0.01 * shell.gravity, shell_mat, trail_pool, 1000::ms, 30, 1);
        }
    }
