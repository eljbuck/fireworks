...
public class Peony extends GGen {
    300 => int NUM_STARS;
    4 => int TRAIL_LENGTH;
    Particle shell --> this;
    vec3 initial_pos;
    ...
    fun void init() {
        shell.init(gravity, shell_mat, trail_pool, 350::ms, 8, 1);
        for (0 => int i; i < NUM_STARS; 1 +=> i) {
            stars[i] --> this;
            rand_unit_vec() * Math.random2f(0.0, 0.6) => new_velocities[i];
            if (Math.random2(0, 1)) {
                stars[i].init(0.01 * shell.gravity, shell_mat, trail_pool, 350::ms, 0, 1);
            } else {
                stars[i].init(0.01 * shell.gravity, second_col, trail_pool, 350::ms, 0, 1);
            }
    ...
    fun void boom(dur del) {
        ...
        for (0 => int i; i < NUM_STARS; 1 +=> i) {
            ...
            spork ~ stars[i].apply_force(-.02 * new_velocities[i]);
        }
        delay(1::second);
        star_fade(1::second);
